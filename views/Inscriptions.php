...
<body>
    ...
    <?php if ($addClientOk) { ?>
        <p class="text-center">Vous avez bien été enregistré</p>
        <a href="welcome.php" class="justify-content-center btn btn-outline-success">Ajouter un nouveau Client</a>
    <?php } else { ?>
        <div class="row p-4">
            <h1 class="text-center">Inscription</h1>

            <form class="d-flex justify-content-center pt-5" action="" method="POST">
                <div class="col-4  mb-3 form-group">
                    <label for="nom" class="fs-4 form-label mt-3">Nom : </label><span class="text-danger">
                        <?=
                        $arrayError["nom"] ?? " ";
                        ?>
                    </span>
                    ...
                    <label for="prenom" class="fs-4 form-label mt-3">Prénom : </label><span class="text-danger">
                        <?=
                        $arrayError["prenom"] ?? "";
                        ?>
                    </span>
                    ...
                    <label for="phone" class="fs-4 form-label mt-3">N° de téléphone : </label><span class="text-danger">
                        <?=
                        $arrayError["phone"] ?? " ";
                        ?>
                    </span>
                    ...
                    <label for="mail" class="fs-4 form-label mt-3">Adresse mail : </label><span class="text-danger">
                        <?=
                        $arrayError["mail"] ?? " ";
                        ?>
                    </span>
                    <input value="<?= isset($_POST["mail"]) ? htmlspecialchars($_POST["mail"]) : "" ?>" name="mail" type="email" class="form-control" id="mail" placeholder="Ex : Dupont.Jean@gmail.com" required>

                    <label for="password" class="fs-4 form-label mt-3">Mot de passe : </label><span class="text-danger">
                        <?=
                        $arrayError["password"] ?? " ";
                        ?>
                    </span>
                    <input value="<?= isset($_POST["password"]) ? htmlspecialchars($_POST["password"]) : "" ?>" name="password" type="text" class="form-control" id="password" placeholder="Ex :**********" required>

                    <label for="confPassword" class="fs-4 form-label mt-3">Confirmation Mot de passe : </label><span class="text-danger">
                        <?=
                        $arrayError["confPassword"] ?? " ";
                        ?>
                    </span>
                    <input value="<?= isset($_POST["confPassword"]) ? htmlspecialchars($_POST["confPassword"]) : "" ?>" name="confPassword" type="text" class="form-control" id="confPassword" placeholder="Ex :**********" required>

                    <div class="pt-4 d-flex justify-content-center">
                        <button type="submit" href="welcome.php" class="btn btn-success">Enregistrer le nouveau Client</button>
                        <a href="pageInscription.php" type="submit" value="ok" class="m-2 col-1 btn btn-danger ">Retour </a>
